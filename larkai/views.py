...
def home(request):
    ...
    if request.method=='POST':
        ...
        with open("/home/larkai/Downloads/larkai/info.json", "w") as write_file:
...
def patient_count():
    ...
    path = "/home/larkai/Downloads/larkai/patient_Data" #path for patient_data folder
...
def fetch_value(request):
    try:
        ...
        nline=[]
        global x,prev_value
        with open('/home/larkai/Downloads/larkai/ecg_data.csv','r') as ecg:
            ecg_reader=csv.reader(ecg)
            #time.sleep(5)
            
            
            file=open('/home/larkai/Downloads/larkai/ecg_data.csv')
            reader=csv.reader(file)
            lines=len(list(reader))
            x=lines-10
...
##            print("test")
            ...
            process.kill()
            pcg=subprocess.Popen('python3 /home/larkai/Downloads/larkai/spi2.py',shell=True,preexec_fn=os.setsid)
...
def reading(request):
    global extra,ecg
    if(extra==False):
        ecg=subprocess.Popen('python3 /home/larkai/Downloads/larkai/spi.py',shell=True,preexec_fn=os.setsid)
        time.sleep(1)
        print("Spi 1 started")
        extra = True
...
def loading(request):
    ...
    process.kill()
    subprocess.Popen('python3 /home/larkai/Downloads/larkai/Cardio/run.py',shell=True)
...
def final(request):
    ...
    #ecg.terminate()
    print("Spi 2 ended")
    subprocess.Popen('python3 /home/larkai/Downloads/larkai/Cardio/pdf_gen.py',shell=True)
    time.sleep(5)
    #messages.info(request,fname)
    #time.sleep(40)
